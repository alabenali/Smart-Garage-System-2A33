...
class UserController {
    ...
    private function getGarageConnection(): PDO {
        $dsn = 'mysql:host=localhost;dbname=smart_garage;charset=utf8mb4';
    ...
    private function getClientRendezvousData(int $clientId, string $clientEmail): array {
        $data = [
            'upcoming' => [],
            'past' => [],
            'all' => [],
        ...
        try {
            $garageDb = $this->getGarageConnection();
            $hasClientColumn = $this->garageColumnExists($garageDb, 'rendezvous_digital', 'id_client');

            $where = $hasClientColumn
                ? '(r.id_client = :id_client OR r.email_client = :email)'
                : 'r.email_client = :email';
            $clientSelect = $hasClientColumn ? 'r.id_client,' : 'NULL AS id_client,';
            ...
            foreach ($rows as $row) {
                ...
                $dateTs = !empty($row['date_heure']) ? strtotime((string) $row['date_heure']) : false;
                $isClosed = in_array($this->normalizeRdvStatus($status), ['termine', 'annule'], true);
                $isFuture = $dateTs !== false && $dateTs >= $now && !$isClosed;

                $item = $row;
                $item['is_future'] = $isFuture;
                $item['vehicle_label'] = trim((string) (($row['marque'] ?? '') . ' ' . ($row['modele'] ?? '') . ' ' . ($row['immatriculation'] ?? '')));
            ...
            usort($data['upcoming'], static function (array $a, array $b): int {
                return strtotime((string) ($a['date_heure'] ?? '')) <=> strtotime((string) ($b['date_heure'] ?? ''));
            });
            ...
            $data['loyalty'] = $this->getClientLoyaltyData($garageDb, $clientEmail);
        } catch (Throwable $e) {
            $data['load_error'] = 'Impossible de charger vos rendez-vous pour le moment.';
        }

        return $data;
    }

    private function getClientLoyaltyData(PDO $garageDb, string $clientEmail): array {
        $goal = 200;
        $loyalty = [
            'points' => 0,
            'total' => 0,
        ...
        try {
            $stmt = $garageDb->prepare('SELECT * FROM loyalty_account WHERE client_email = :email LIMIT 1');
            $stmt->execute([':email' => strtolower(trim($clientEmail))]);
            $account = $stmt->fetch();
            ...
            $loyalty['total'] = max(0, (int) ($account['points_total'] ?? 0));
            $loyalty['used'] = max(0, (int) ($account['points_utilises'] ?? 0));
            $loyalty['tier'] = (string) ($account['palier_actuel'] ?? 'Bronze');
            $loyalty['progress_pct'] = max(0, min(100, (int) round(($points / $goal) * 100)));
            $loyalty['missing'] = max(0, $goal - $points);
            $loyalty['unlocked'] = $points >= $goal;

            $history = $garageDb->prepare(
                'SELECT type, points, description, date_transaction
                 FROM loyalty_transactions
                 WHERE loyalty_id = :id
                 ORDER BY date_transaction DESC, id DESC
                 LIMIT 5'
            );
            $history->execute([':id' => (int) $account['id']]);
            $loyalty['history'] = $history->fetchAll();
        } catch (Throwable $e) {
            return $loyalty;
        }
    ...
    private function garageColumnExists(PDO $garageDb, string $table, string $column): bool {
        if (!preg_match('/^[A-Za-z0-9_]+$/', $table) || !preg_match('/^[A-Za-z0-9_]+$/', $column)) {
            return false;
        }

        try {
            $stmt = $garageDb->query("SHOW COLUMNS FROM {$table} LIKE " . $garageDb->quote($column));
    ...
    private function normalizeRdvStatus(string $status): string {
        $status = strtolower(trim($status));
        $status = str_replace(['é', 'è', 'ê', 'ë', 'à', 'â', 'ù', 'û', 'ç'], ['e', 'e', 'e', 'e', 'a', 'a', 'u', 'u', 'c'], $status);
    ...
    public function resendVerification(): void {
        ...
        header('Location: /integration/client/controllers/UserController.php?action=showVerifyRegister');
        exit;
    }
    ...
    public function register(): void {
        ...
        if (!empty($errors)) {
            $_SESSION['errors'] = $errors;
            $_SESSION['old']    = $_POST;
            header('Location: /integration/client/controllers/UserController.php?action=showRegister');
            exit;
        }
    ...
    public function forgotPassword(): void {
        ...
        header('Location: /integration/client/controllers/UserController.php?action=showVerifyCode');
        exit;
    }
    ...
    public function verifyResetCode(): void {
        ...
        header('Location: /integration/client/controllers/UserController.php?action=showResetPassword');
        exit;
    }
    ...
    public function updateProfile(): void {
        ...
        if (!empty($errors)) {
            $_SESSION['errors'] = $errors;
            header('Location: /integration/client/controllers/UserController.php?action=showProfile');
            exit;
        }
    ...
    public function showMyRendezvous(): void {
        if (isset($_SESSION['admin_id']) || (($_SESSION['role'] ?? '') === 'admin')) {
            header('Location: /integration/client/controllers/AdminController.php?action=showDashboard');
            exit;
        }
        ...
        $rdvPageData = $this->getClientRendezvousData((int) $user['id'], (string) $user['email']);
        require_once __DIR__ . '/../views/frontoffice/my_rendezvous.php';
    }
    ...
    public function googleCallback(): void {
        ...
        if ($row) {
            ...
            if (empty($row['google_id'])) {
                $pdo->prepare('UPDATE user SET google_id = :gid WHERE id = :id')
...
$allowedActions = [
    ...
    'showDashboard','showMyRendezvous','logout',
    'getFaceImage',
