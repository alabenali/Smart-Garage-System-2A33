...
$email  = htmlspecialchars($_SESSION['pending_register']['email']);
$prenom = htmlspecialchars($_SESSION['pending_register']['prenom']);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Confirmer l'inscription - Smart Garage</title>
...
<div class="auth-card">
    ...
    <p class="hint">
        Bonjour <strong><?= $prenom ?></strong>, un code à 6 chiffres a été envoyé à<br>
        <strong><?= $email ?></strong>
    </p>

    <form method="POST" action="/integration/client/controllers/UserController.php?action=verifyRegisterCode" id="codeForm">
        ...
        <input type="hidden" name="register_code" id="register_code">
        <button type="submit" class="btn-primary full" id="submitBtn" disabled>
            <i class="fas fa-user-check"></i> Confirmer et créer mon compte
        </button>
    </form>

    <div class="links">
        <p>Pas reçu le code ? <a href="/integration/client/controllers/UserController.php?action=resendRegisterCode">Renvoyer</a></p>
        <p><a href="/integration/client/views/frontoffice/register.php"><i class="fas fa-arrow-left"></i> Recommencer l'inscription</a></p>
    </div>
</div>
<script>
const digits    = document.querySelectorAll('.code-digit');
const hidden    = document.getElementById('register_code');
