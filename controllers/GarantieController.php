<?php
// ============================================
// Contrôleur Garantie – Dashboard + API JSON
// ============================================
...
class GarantieController
{
    ...
    // ── Dashboard garanties (BackOffice) ──
    ...
    // ── Créer les garanties après une commande ──
    public function createAfterCommande(int $id_commande, array $items, int $id_client)
    {
        ...
        foreach ($items as $item) {
            ...
            // Récupérer la durée de garantie de la pièce
            $duree = isset($item['duree_garantie_mois'])
                ? (int) $item['duree_garantie_mois']
                : $this->model->getDureePiece($id_piece);

            // Ignorer les pièces sans garantie
            if ($duree <= 0) continue;
            ...
            try {
                ...
                // Ne pas bloquer la commande si une garantie échoue
                error_log('Erreur création garantie pièce #' . $id_piece . ' : ' . $e->getMessage());
            }
        }
    }

    // ── API JSON : Garanties d'un client ──
    ...
    // ── API JSON : Marquer une garantie comme remplacée ──
    public function marquerRemplacee()
    {
        ...
        try {
            ...
            header('Location: index.php?action=manageGaranties&success=' . rawurlencode('Garantie #' . $id_garantie . ' marquée comme remplacée.'));
        } catch (Throwable $e) {
    ...
    // ── API JSON : Test alertes (sans envoi SMS) ──
