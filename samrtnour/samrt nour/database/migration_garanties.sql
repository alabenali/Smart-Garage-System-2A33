...
ALTER TABLE pieces ADD COLUMN garantie_mois INT DEFAULT 1 COMMENT 'Duree garantie en mois, minimum 1 mois';
...
CREATE TABLE IF NOT EXISTS garanties (
    ...
    date_expiration     DATE GENERATED ALWAYS AS (DATE_ADD(date_pose, INTERVAL 30 DAY)) STORED,
...
UPDATE pieces SET garantie_mois = 1  WHERE LOWER(nom) LIKE '%filtre%' OR LOWER(nom) LIKE '%huile%' OR LOWER(nom) LIKE '%liquide%';
