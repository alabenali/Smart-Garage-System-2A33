    ...
    public function convertToCommande($id_panier, $client_data, $payment_method)
    {
        try {
            ...
            $insCmd = $this->conn->prepare(
                'INSERT INTO commandes (id_piece, id_client, nom_client, prenom_client, telephone, email_client, id_vehicle, id_rdv, quantite, montant_total, statut, payment_method, payment_status, montant_ht, tva, frais_livraison, montant_ttc, source, id_panier, note)
                 VALUES (:ip, :cid, :nc, :pc, :tel, :email, :vehicle, :rdv, :q, :mt, :st, :pm, :ps, :mht, :tva, :fl, :ttc, :src, :pan, :note)'
            );
            $insCmd->execute([
                ':ip' => (int)$first['id_piece'], ':cid' => !empty($client_data['id_client']) ? (int)$client_data['id_client'] : null,
                ':nc' => $client_data['nom_client'] ?? '', ':pc' => $client_data['prenom_client'] ?? '',
                ':tel' => $client_data['telephone'] ?? '', ':email' => $client_data['email_client'] ?? ($client_data['email'] ?? null),
                ':vehicle' => !empty($client_data['id_vehicle']) ? (int)$client_data['id_vehicle'] : null,
                ':rdv' => !empty($client_data['id_rdv']) ? (int)$client_data['id_rdv'] : null,
                ':q' => $summary['nb_articles'], ':mt' => $summary['total_ttc'],
