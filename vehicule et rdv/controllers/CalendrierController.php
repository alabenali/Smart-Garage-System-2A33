...
class CalendrierController
{
    ...
    private ?bool $hasClientColumnCache = null;
    ...
    public function frontCalendar(): void
    {
        ...
        $clientData = $this->rdvService->getClientWithVehicles((int) $_SESSION['user_id']);
        $clientVehicles = (array) ($clientData['vehicles'] ?? []);
        if (empty($clientVehicles)) {
            $_SESSION['rdv_errors'] = ['Ajoutez d abord un vehicule avant de prendre un rendez-vous.'];
            header('Location: index.php?action=addVehicle&next=rdv');
            exit;
        }
        $selectedVehicleId = isset($_GET['id_vehicule']) ? (int) $_GET['id_vehicule'] : (int) ($clientVehicles[0]['id_vehicule'] ?? 0);
    ...
    public function frontCreate(): void
    {
        ...
        $payload['id_client'] = (int) $_SESSION['user_id'];
        ...
        $relation = $this->rdvService->validateClientVehicleRelation(
            (int) $payload['id_client'],
            (int) $payload['id_vehicule'],
            true
        );
        if (!$relation['valid']) {
            $_SESSION['rdv_errors'] = $relation['errors'];
            $_SESSION['rdv_old'] = $_POST;
            header('Location: index.php?action=frontCalendar&selected_date=' . $slotDate);
            exit;
        }
        ...
        $rdvId = $this->createRendezvous([
            ...
            'telephone_client' => '',
            'email_client' => null,
            'id_vehicle' => (int) $payload['id_vehicule'],
        ...
        $this->sendEmailConfirmation($rdvId);
    ...
    private function apiRendezVousUrgents(): void
    {
        ...
        $clientSelect = $this->hasClientColumn() ? 'r.id_client,' : 'NULL AS id_client,';
    ...
    private function apiRendezVousCreate(): void
    {
        ...
        $idClient = isset($payload['id_client']) ? (int) $payload['id_client'] : null;
        $idVehicle = $this->resolveVehicleIdInput($payload);
        ...
        $relationRequired = $idClient !== null || $idVehicle !== null;
        try {
            $relation = $this->rdvService->validateClientVehicleRelation($idClient, $idVehicle, $relationRequired);
        } catch (Throwable $e) {
            $this->jsonResponse(['success' => false, 'message' => 'Validation client/vehicule indisponible'], 500);
            return;
        }

        if (!$relation['valid']) {
            $this->jsonResponse(['success' => false, 'message' => implode(' ', $relation['errors'])], 422);
            return;
        }

        $client = is_array($relation['client']) ? $relation['client'] : [];
        ...
        $data = [
            ...
            'id_client' => $idClient,
            'nom_client' => $this->sanitize((string) ($payload['nom_client'] ?? ($client['nom'] ?? ''))),
            'prenom_client' => $this->sanitize((string) ($payload['prenom_client'] ?? ($client['prenom'] ?? ''))),
            'telephone_client' => preg_replace('/\D+/', '', (string) ($payload['telephone_client'] ?? ($client['telephone'] ?? ''))),
            'email_client' => trim((string) ($payload['email_client'] ?? ($client['email'] ?? ''))),
        ...
        try {
            ...
            $this->jsonResponse(['success' => false, 'message' => 'Erreur base de donnees pendant la creation RDV'], 500);
            return;
        }
    ...
    private function apiRendezVousUpdate(int $idRdv): void
    {
        ...
        $idClient = array_key_exists('id_client', $payload) ? (int) $payload['id_client'] : (isset($existing['id_client']) ? (int) $existing['id_client'] : null);
        $idVehicle = $this->resolveVehicleIdInput($payload, isset($existing['id_vehicle']) ? (int) $existing['id_vehicle'] : null);
        $relationRequired = $idClient !== null || $idVehicle !== null;
        try {
            $relation = $this->rdvService->validateClientVehicleRelation($idClient, $idVehicle, $relationRequired);
        } catch (Throwable $e) {
    ...
    public function backCalendar(): void
    {
        ...
        if (isset($_GET['id_client']) && (int) $_GET['id_client'] > 0 && empty($manualOld['id_client'])) {
            $manualOld['id_client'] = (int) $_GET['id_client'];
        }
        $manualClients = $this->listClientsForSelect();
        $manualVehicles = $this->listVehiclesForSelect();
    ...
    public function backEditRdv(): void
    {
        $idRdv = isset($_GET['id']) ? (int) $_GET['id'] : (int) ($_POST['id_rdv'] ?? 0);
        $rdv = $this->findDetailedById($idRdv);
        if (!$rdv) {
            header('Location: index.php?action=backRdvList&error=RDV introuvable');
            exit;
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($idCreneau <= 0 && !empty($_POST['date_heure_manual'])) {
                if ($this->isValidDateTime((string) $_POST['date_heure_manual'])) {
                    $idCreneau = $this->findOrCreateByDateTime((string) $_POST['date_heure_manual']);
                } else {
                    $errors[] = 'Date/heure manuelle invalide.';
                }
            }

            $slot = $idCreneau > 0 ? $this->findCreneauById($idCreneau) : null;
            if (!$slot) {
                $errors[] = 'Creneau introuvable.';
            }

            $idClient = isset($_POST['id_client']) && (int) $_POST['id_client'] > 0 ? (int) $_POST['id_client'] : null;
            $idVehicle = isset($_POST['id_vehicule']) && (int) $_POST['id_vehicule'] > 0 ? (int) $_POST['id_vehicule'] : null;
            $relationRequired = $idClient !== null || $idVehicle !== null;
            $relation = ['valid' => true, 'errors' => [], 'client' => null, 'vehicle' => null];
            try {
                $relation = $this->rdvService->validateClientVehicleRelation($idClient, $idVehicle, $relationRequired);
            } catch (Throwable $e) {
                $errors[] = 'Validation client/vehicule indisponible.';
            }

            if (!$relation['valid']) {
                $errors = array_merge($errors, $relation['errors']);
            }

            $typeIntervention = $this->sanitize((string) ($_POST['type_intervention'] ?? ''));
            $description = $this->sanitize((string) ($_POST['description_panne'] ?? ''));
            $circ = $this->sanitize((string) ($_POST['circonstances_panne'] ?? ''));
            $statut = trim((string) ($_POST['statut'] ?? 'En attente'));
            $temoins = $this->normalizeTemoinsInput($_POST['temoins_panne'] ?? []);

            if (!in_array($typeIntervention, $this->allowedInterventions, true)) {
                $errors[] = 'Type intervention invalide.';
            }
            if ($description === '') {
                $errors[] = 'La description est obligatoire.';
            }
            if ($circ !== '' && !in_array($circ, $this->allowedCircumstances, true)) {
            ...
            if (empty($errors) && $slot) {
                $client = is_array($relation['client']) ? $relation['client'] : [];
                $data = [
                    'id_creneau' => (int) $slot['id_creneau'],
                    'id_client' => $idClient,
                    'nom_client' => $this->sanitize((string) ($_POST['nom_client'] ?? ($client['nom'] ?? ''))),
                    'prenom_client' => $this->sanitize((string) ($_POST['prenom_client'] ?? ($client['prenom'] ?? ''))),
                    'telephone_client' => preg_replace('/\D+/', '', (string) ($_POST['telephone_client'] ?? ($client['telephone'] ?? ''))),
                    'email_client' => trim((string) ($_POST['email_client'] ?? ($client['email'] ?? ''))),
                    'id_vehicle' => $idVehicle,
                    'type_intervention' => $typeIntervention,
                    'description_panne' => $description,
                    'circonstances_panne' => $circ,
                    'temoins_panne' => $temoins,
                    'panne_data_json' => $this->buildPanneJson($_POST, $typeIntervention, $temoins, (string) ($rdv['panne_data_json'] ?? '')),
                    'photos_json' => $rdv['photos_json'] ?? json_encode([], JSON_UNESCAPED_UNICODE),
                    'remise_eco_appliquee' => ((int) ($slot['est_heure_creuse'] ?? 0) === 1) ? 15.00 : 0.00,
                    'statut' => $statut,
                    'notes' => $this->sanitize((string) ($_POST['notes'] ?? '')),
                ];
                ...
                if ($this->updateRendezvous($idRdv, $data)) {
                    ...
                    $rdv = $this->findDetailedById($idRdv) ?: $rdv;
                } else {
                    $errors[] = 'Mise a jour impossible.';
                }
            } else {
                $rdv = array_merge($rdv, $_POST, ['id_vehicle' => $idVehicle, 'id_client' => $idClient, 'id_creneau' => $idCreneau]);
            }
        }

        $clients = $this->listClientsForSelect();
        $vehicles = $this->listVehiclesForSelect();
        require __DIR__ . '/../views/back/rdv_edit.php';
    }
    ...
    private function validateRdvRequest(array $input, bool $manualMode): array
    {
        ...
        $telephoneClient = '';
        $emailClient = '';
        $idClient = isset($input['id_client']) && (int) $input['id_client'] > 0 ? (int) $input['id_client'] : null;
        $idVehicule = isset($input['id_vehicule']) ? (int) $input['id_vehicule'] : (isset($input['id_vehicle']) ? (int) $input['id_vehicle'] : 0);
        $vehicle = null;
        ...
        if ($manualMode) {
            ...
            if ($idClient === null && $nomClient === '') {
                $errors[] = 'Le nom du client est obligatoire.';
            }
            if ($idClient === null && $prenomClient === '') {
                $errors[] = 'Le prénom du client est obligatoire.';
            }
            if ($idClient === null && !preg_match('/^\d{8}$/', $telephoneClient)) {
                $errors[] = 'Le téléphone doit contenir exactement 8 chiffres (format tunisien).';
            }
            if ($idClient === null && $emailClient === '') {
                $errors[] = 'L\'email du client est obligatoire.';
            } elseif ($emailClient !== '' && !filter_var($emailClient, FILTER_VALIDATE_EMAIL)) {
                $errors[] = 'Email invalide.';
            }
            if ($idClient !== null) {
                try {
                    $selectedClient = $this->rdvService->findClientById($idClient);
                    if (!$selectedClient) {
                        $errors[] = 'Client introuvable.';
                    } else {
                        $nomClient = $selectedClient['nom'] ?? $nomClient;
                        $prenomClient = $selectedClient['prenom'] ?? $prenomClient;
                        $telephoneClient = preg_replace('/\D+/', '', (string) ($selectedClient['telephone'] ?? $telephoneClient));
                        $emailClient = $selectedClient['email'] ?? $emailClient;
                    }
                } catch (Throwable $e) {
            ...
            if ($idVehicule > 0) {
                $existingVehicle = $this->rdvService->findVehicleById($idVehicule);
                if (!$existingVehicle) {
                    $errors[] = 'Vehicule introuvable.';
                }
                if ($idClient !== null) {
                    $relation = $this->rdvService->validateClientVehicleRelation($idClient, $idVehicule, true);
                    if (!$relation['valid']) {
                        $errors = array_merge($errors, $relation['errors']);
                    }
                }
            } elseif ($immatriculation !== '') {
                $existingVehicle = $this->findVehicleByImmatriculation($immatriculation);
            } else {
                $errors[] = 'Selectionnez un vehicule ou renseignez une immatriculation.';
            }
            ...
            if ($existingVehicle) {
                $idVehicule = (int) ($existingVehicle['id'] ?? $idVehicule);
                $vehicle += [
                    'id_client' => isset($existingVehicle['id_client']) ? (int) $existingVehicle['id_client'] : $idClient,
                    'immatriculation' => $existingVehicle['immatriculation'] ?? $immatriculation,
    ...
    private function formatRdvApiRow(array $row): array
    {
        ...
        return [
            'id' => (int) ($row['id_rdv'] ?? 0),
            'id_rdv' => (int) ($row['id_rdv'] ?? 0),
            'id_client' => isset($row['id_client']) ? (int) $row['id_client'] : null,
            'id_vehicule' => isset($row['id_vehicle']) ? (int) $row['id_vehicle'] : null,
            'id_vehicle' => isset($row['id_vehicle']) ? (int) $row['id_vehicle'] : null,
            ...
            'vehicule' => !empty($row['id_vehicle']) ? [
                'id_vehicule' => (int) $row['id_vehicle'],
    ...
    private function resolveVehicleIdInput(array $payload, ?int $fallback = null): ?int
    {
        if (array_key_exists('id_vehicule', $payload)) {
            $id = (int) $payload['id_vehicule'];
            return $id > 0 ? $id : null;
        }

        if (array_key_exists('id_vehicle', $payload)) {
            $id = (int) $payload['id_vehicle'];
            return $id > 0 ? $id : null;
        }
    ...
    private function sendEmailConfirmation(int $rdvId): void
    {
        try {
            ...
            $rdv = $this->findDetailedById($rdvId);
            if (!$rdv || empty($rdv['email_client'])) {
                return;
            }

            $clientName = trim((string) (($rdv['prenom_client'] ?? '') . ' ' . ($rdv['nom_client'] ?? '')));
            $dateHeure = !empty($rdv['date_heure']) ? date('d/m/Y H:i', strtotime((string) $rdv['date_heure'])) : '-';
            $vehicle = trim((string) (($rdv['marque'] ?? '') . ' ' . ($rdv['modele'] ?? '') . ' ' . ($rdv['immatriculation'] ?? '')));
            ...
            $mail->addAddress((string) $rdv['email_client'], $clientName !== '' ? $clientName : 'Client Smart Garage');
            $mail->isHTML(true);
            $mail->Subject = 'Confirmation de votre rendez-vous Smart Garage';
            $mail->Body = '<p>Bonjour ' . htmlspecialchars($clientName, ENT_QUOTES, 'UTF-8') . ',</p>'
                . '<p>Votre rendez-vous est confirme pour le <strong>' . htmlspecialchars($dateHeure, ENT_QUOTES, 'UTF-8') . '</strong>.</p>'
                . '<p>Intervention : ' . htmlspecialchars((string) ($rdv['type_intervention'] ?? ''), ENT_QUOTES, 'UTF-8') . '</p>'
                . ($vehicle !== '' ? '<p>Vehicule : ' . htmlspecialchars($vehicle, ENT_QUOTES, 'UTF-8') . '</p>' : '')
                . '<p>Merci de votre confiance.</p>';
            $mail->AltBody = "Votre rendez-vous Smart Garage est confirme pour le {$dateHeure}. Intervention: " . (string) ($rdv['type_intervention'] ?? '');
            $mail->send();
        } catch (Throwable $e) {
            // L email ne doit jamais bloquer la creation du RDV.
        }
    }
    ...
    private function getFiltered(array $filters, int $limit, int $offset): array
    {
        ...
        if (!empty($filters['client_id']) && $this->hasClientColumn()) {
    ...
    private function listVehiclesForSelect(): array
    {
        $clientSelect = $this->hasVehicleClientColumn() ? 'id_client' : 'NULL AS id_client';
        $stmt = $this->db->query("SELECT id, {$clientSelect}, marque, modele, immatriculation, kilometrage, carburant FROM vehicle ORDER BY marque ASC, modele ASC, immatriculation ASC");
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    private function hasClientColumn(): bool
    {
        if ($this->hasClientColumnCache !== null) {
            return $this->hasClientColumnCache;
        }

        try {
            $stmt = $this->db->query("SHOW COLUMNS FROM rendezvous_digital LIKE 'id_client'");
            $this->hasClientColumnCache = (bool) $stmt->fetch(PDO::FETCH_ASSOC);
        } catch (Throwable $e) {
            $this->hasClientColumnCache = false;
        }
