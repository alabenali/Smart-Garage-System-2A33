...
function generateWeeklyReport(): array
{
    ...
    $emailOk = (bool) ($emailResult['success'] ?? false);
    $emailError = (string) ($emailResult['error'] ?? '');
    $logs[] = "Email : " . ($emailOk ? 'SUCCESS' : 'FAIL');
    if (!$emailOk && $emailError !== '') {
        $logs[] = "Erreur email : " . $emailError;
    }
    ...
    $logLine = '[' . date('Y-m-d H:i:s') . '] | RDV: ' . $stats['total_rdv']
             . ' | EMAIL: ' . ($emailOk ? 'SUCCESS' : 'FAIL')
             . (!$emailOk && $emailError !== '' ? ' | ERROR: ' . $emailError : '')
             . ' | TIME: ' . $elapsed . 's';
...
{
    $fallback = generateLocalAIComment($stats);

    $apiKey = defined('OPENAI_API_KEY') ? OPENAI_API_KEY : '';
    if (empty($apiKey) || $apiKey === 'YOUR_REAL_API_KEY') {
        return $fallback;
    }
...
/**
 * Analyse locale de secours quand l API IA n est pas configuree.
 */
function generateLocalAIComment(array $stats): string
{
    $total = (int) ($stats['total_rdv'] ?? 0);
    $occupation = (float) ($stats['taux_occupation'] ?? 0);
    $annulation = (float) ($stats['taux_annulation'] ?? 0);
    $urgents = (int) ($stats['rdv_urgents'] ?? 0);
    $previous = (int) ($stats['semaine_precedente_total'] ?? 0);
    $delta = $total - $previous;

    $trend = $delta > 0
        ? "L'activite progresse de {$delta} rendez-vous par rapport a la semaine precedente."
        : ($delta < 0
            ? "L'activite recule de " . abs($delta) . " rendez-vous par rapport a la semaine precedente."
            : "L'activite reste stable par rapport a la semaine precedente.");

    $pressure = $occupation >= 80
        ? "Le taux d'occupation est eleve ({$occupation}%), ce qui indique une forte pression sur l'atelier."
        : "Le taux d'occupation reste maitrise ({$occupation}%), avec une capacite encore disponible.";

    $risk = $annulation > 15
        ? "Le taux d'annulation de {$annulation}% demande un suivi des confirmations client."
        : "Le taux d'annulation de {$annulation}% reste acceptable.";

    $recommendation = $urgents > 0
        ? "Recommandation : prioriser les {$urgents} rendez-vous urgents et reserver des creneaux de diagnostic rapide."
        : "Recommandation : maintenir le suivi des confirmations et surveiller les pics horaires pour optimiser la charge.";

    return "{$trend} {$pressure} {$risk} {$recommendation}";
}
 ...
 */
function sendReportEmail(array $stats, string $aiComment, string $pdfPath, string $periode): array
{
    if (!defined('SMTP_USER') || trim((string) SMTP_USER) === '') {
        return ['success' => false, 'error' => 'SMTP_USER manquant.'];
    }

    if (!defined('SMTP_PASS') || trim((string) SMTP_PASS) === '') {
        return ['success' => false, 'error' => 'SMTP_PASS manquant. Configurez un mot de passe d application Gmail dans config/secrets.php.'];
    }

    if (!defined('GERANT_EMAIL') || !filter_var(GERANT_EMAIL, FILTER_VALIDATE_EMAIL)) {
        return ['success' => false, 'error' => 'GERANT_EMAIL invalide.'];
    }

    if (!file_exists($pdfPath)) {
        return ['success' => false, 'error' => 'PDF introuvable avant envoi.'];
    }
    ...
    try {
        ...
        error_log('[Smart Garage] Email error: ' . $message);
