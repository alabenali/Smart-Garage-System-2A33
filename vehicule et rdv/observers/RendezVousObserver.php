...
class RendezVousObserver
{
    ...
    private function buildContext(array $rdv, ?int $rdvId): array
    {
        ...
        try {
            $rdvService = new RdvService($this->db);
            $context = array_merge($context, $rdvService->buildScoringContext($rdv));
        } catch (Throwable $e) {
            // Le scoring historique enrichi ne doit jamais bloquer le RDV.
        }
