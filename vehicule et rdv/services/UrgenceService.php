...
class UrgenceService
{
    ...
    public function calculerScoreAvance(array $context): array
    {
        ...
        $clientHistoryMin = (int) ($advanced['client_history_min'] ?? 3);
        $clientHistoryScore = (int) ($advanced['client_history_score'] ?? 1);
        $clientNoShowScore = (int) ($advanced['client_no_show_score'] ?? 1);
        $vehicleHealthThreshold = (int) ($advanced['vehicle_health_threshold'] ?? 45);
        $vehicleHealthScore = (int) ($advanced['vehicle_health_score'] ?? 2);
        $failureFrequencyMin = (int) ($advanced['vehicle_failure_frequency_min'] ?? 3);
        $failureFrequencyScore = (int) ($advanced['vehicle_failure_frequency_score'] ?? 1);
        ...
        if (isset($context['client_history_count'])) {
            $clientHistoryCount = (int) $context['client_history_count'];
            if ($clientHistoryCount >= $clientHistoryMin && $clientHistoryScore > 0) {
                $score += $clientHistoryScore;
                $details['historique_client'] = $clientHistoryScore;
            }
        }

        if (!empty($context['client_no_show_count']) && $clientNoShowScore > 0) {
            $score += $clientNoShowScore;
            $details['client_rdv_annules'] = $clientNoShowScore;
        }

        if (isset($context['vehicle_health_score'])) {
            $health = (int) $context['vehicle_health_score'];
            if ($health <= $vehicleHealthThreshold && $vehicleHealthScore > 0) {
                $score += $vehicleHealthScore;
                $details['etat_vehicule'] = $vehicleHealthScore;
            }
        }

        if (isset($context['vehicle_failure_frequency'])) {
            $failureFrequency = (int) $context['vehicle_failure_frequency'];
            if ($failureFrequency >= $failureFrequencyMin && $failureFrequencyScore > 0) {
                $score += $failureFrequencyScore;
                $details['frequence_pannes'] = $failureFrequencyScore;
            }
        }
    ...
    private function mergeDefaults(array $config): array
    {
        $defaults = [
            ...
            'advanced' => [
                ...
                'history_min' => 2,
                'history_score' => 1,
                'client_history_min' => 3,
                'client_history_score' => 1,
                'client_no_show_score' => 1,
                'vehicle_health_threshold' => 45,
                'vehicle_health_score' => 2,
                'vehicle_failure_frequency_min' => 3,
                'vehicle_failure_frequency_score' => 1,
            ],
            'urgent_min_score' => 7,
            'max_score' => 10,
            'broadcast' => [
