...
<div class="sg-form-wrap mt-4">
    ...
    <form action="index.php?action=backCreateManualRdv" method="POST" class="cal-form-grid">
        <div class="sg-form-group full-width">
            ...
            <select name="id_client" id="manualClientSelect">
                <option value="">-- Client non rattache --</option>
                <?php foreach (($manualClients ?? []) as $client): ?>
                    <?php
                    ...
                    <option value="<?php echo $clientId; ?>"
                            ...
                            <?php echo ((int) ($manualOld['id_client'] ?? 0) === $clientId) ? 'selected' : ''; ?>>
        ...
        <div class="sg-form-group full-width">
            <label>Vehicule existant</label>
            <select name="id_vehicule" id="manualVehicleSelect">
                <option value="">-- Selectionner ou creer via immatriculation --</option>
                <?php foreach (($manualVehicles ?? []) as $vehicle): ?>
                    <?php
                    $vehicleId = (int) ($vehicle['id'] ?? 0);
                    $vehicleClientId = (int) ($vehicle['id_client'] ?? 0);
                    $vehicleLabel = trim(($vehicle['marque'] ?? '') . ' ' . ($vehicle['modele'] ?? '') . ' - ' . ($vehicle['immatriculation'] ?? ''));
                    ?>
                    <option value="<?php echo $vehicleId; ?>"
                            data-client-id="<?php echo $vehicleClientId; ?>"
                            data-immat="<?php echo htmlspecialchars($vehicle['immatriculation'] ?? ''); ?>"
                            <?php echo ((int) ($manualOld['id_vehicule'] ?? 0) === $vehicleId) ? 'selected' : ''; ?>>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const vehicleSelect = document.getElementById('manualVehicleSelect');
    ...
    const telephone = form.querySelector('[name="telephone_client"]');
    ...
    function syncClientFields() {
        ...
        if (nom) nom.value = option.dataset.nom || nom.value;
        if (prenom) prenom.value = option.dataset.prenom || prenom.value;
        if (telephone) telephone.value = (option.dataset.telephone || '').replace(/\D+/g, '').slice(0, 8) || telephone.value;
        if (email) email.value = option.dataset.email || email.value;
    }
    ...
    vehicleSelect.addEventListener('change', function () {
        ...
        if (option && option.dataset.immat && immat) {
            immat.value = option.dataset.immat;
        }
    });
