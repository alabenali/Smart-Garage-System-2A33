...
<div class="sg-form-wrap rdv-filter-wrap">
    <form method="GET" action="index.php" class="rdv-filter-grid">
        ...
        <div class="sg-form-actions">
            ...
            <a class="btn-sg btn-sg-success" href="index.php?action=backRdvExportCsv&status=<?php echo urlencode($filters['status'] ?? ''); ?>&date=<?php echo urlencode($filters['date'] ?? ''); ?>&search=<?php echo urlencode($filters['search'] ?? ''); ?>&client_id=<?php echo (int) ($filters['client_id'] ?? 0); ?>">
                <i class="bi bi-filetype-csv"></i> Export CSV
            </a>
            <a class="btn-sg btn-sg-outline" href="index.php?action=backRdvExportPdf&status=<?php echo urlencode($filters['status'] ?? ''); ?>&date=<?php echo urlencode($filters['date'] ?? ''); ?>&search=<?php echo urlencode($filters['search'] ?? ''); ?>&client_id=<?php echo (int) ($filters['client_id'] ?? 0); ?>">
...
<div class="sg-table-wrap">
    <table class="sg-table">
        ...
        <tbody id="rdvListBody">
            <?php if (empty($rdvs)): ?>
                <tr><td colspan="10" style="text-align:center;">Aucun rendez-vous.</td></tr>
            <?php else: ?>
                <?php foreach ($rdvs as $row): ?>
                    ...
                    <tr class="rdv-summary-row <?php echo $urgenceScore >= 7 ? 'rdv-urgent' : ''; ?>" data-rdv-detail-id="rdv-detail-<?php echo $rdvId; ?>" data-urgence-score="<?php echo $urgenceScore; ?>">
                        ...
                        <td>
                            <?php $clientName = trim((string) (($row['prenom_client'] ?? '') . ' ' . ($row['nom_client'] ?? ''))); ?>
                            <?php if (!empty($row['id_client'])): ?>
                                <a class="vehicle-table-link" href="/integration/client/controllers/AdminController.php?action=showClientDetail&id=<?php echo (int) $row['id_client']; ?>">
                                    <?php echo htmlspecialchars($clientName !== '' ? $clientName : 'Client #' . (int) $row['id_client']); ?>
                                </a>
                            <?php else: ?>
                                <?php echo htmlspecialchars($clientName !== '' ? $clientName : '-'); ?>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if (!empty($row['id_vehicle'])): ?>
                                <a class="vehicle-table-link" href="index.php?action=vehicleDetail&id=<?php echo (int) $row['id_vehicle']; ?>">
                                    <?php echo htmlspecialchars(trim(($row['marque'] ?? '') . ' ' . ($row['modele'] ?? '') . ' ' . ($row['immatriculation'] ?? ''))); ?>
                                </a>
                            <?php else: ?>
                                -
                            <?php endif; ?>
                        ...
                        <td><a class="btn-sg btn-sg-success btn-sg-sm" href="index.php?action=backEditRdv&id=<?php echo $rdvId; ?>"><i class="bi bi-pencil-square"></i></a></td>
                    </tr>
