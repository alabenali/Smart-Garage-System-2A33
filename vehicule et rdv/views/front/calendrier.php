...
<?php if (!empty($_GET['vehicle_added'])): ?>
    <div class="sg-alert sg-alert-success">
        <i class="bi bi-check-circle-fill"></i>
        Vehicule ajoute avec succes. Vous pouvez maintenant prendre votre rendez-vous.
    </div>
<?php endif; ?>
...
<div class="calendar-shell" id="frontCalendarApp" data-month="<?php echo (int) $month; ?>" data-year="<?php echo (int) $year; ?>" data-selected-date="<?php echo htmlspecialchars($selectedDate); ?>">
    ...
    <section class="calendar-block panne-step" data-step-panel="2">
        ...
        <form id="rdvForm" method="POST" action="index.php?action=frontCreateRdv" enctype="multipart/form-data" novalidate>
            ...
            <input type="hidden" name="id_client" value="<?php echo (int) ($_SESSION['user_id'] ?? 0); ?>">
            ...
            $temoinsOptions = [
                ...
                ['value' => 'Voyant moteur allumé', 'icon' => 'bi-exclamation-circle'],
                ['value' => 'Voyant huile allumé', 'icon' => 'bi-droplet'],
                ['value' => 'Voyant batterie allumé', 'icon' => 'bi-battery-half'],
                ['value' => 'Voyant ABS allumé', 'icon' => 'bi-shield-exclamation'],
                ['value' => 'Voyant frein allumé', 'icon' => 'bi-exclamation-triangle'],
                ['value' => 'Voyant température allumé', 'icon' => 'bi-thermometer-high'],
                ['value' => 'Bruit anormal', 'icon' => 'bi-volume-up'],
                ['value' => 'Bruit métallique', 'icon' => 'bi-hammer'],
                ['value' => 'Bruit suspension', 'icon' => 'bi-activity'],
                ['value' => 'Bruit transmission', 'icon' => 'bi-gear-wide-connected'],
                ['value' => 'Vibrations', 'icon' => 'bi-waves'],
                ['value' => 'Fumée', 'icon' => 'bi-cloud-fog2'],
                ['value' => 'Fumée blanche', 'icon' => 'bi-cloud'],
                ['value' => 'Fumée bleue', 'icon' => 'bi-cloud'],
                ['value' => 'Fumée noire', 'icon' => 'bi-cloud'],
                ['value' => 'Fuite de liquide', 'icon' => 'bi-droplet-half'],
                ['value' => "Fuite d'huile", 'icon' => 'bi-droplet-fill'],
                ['value' => 'Fuite de liquide de refroidissement', 'icon' => 'bi-droplet'],
                ['value' => 'Fuite de carburant', 'icon' => 'bi-fuel-pump'],
                ['value' => 'Perte de puissance', 'icon' => 'bi-speedometer'],
                ['value' => 'Démarrage difficile', 'icon' => 'bi-key'],
                ['value' => 'Calage fréquent', 'icon' => 'bi-x-octagon'],
                ['value' => 'Surchauffe moteur', 'icon' => 'bi-thermometer-high'],
                ['value' => 'Odeur de brûlé', 'icon' => 'bi-fire'],
                ['value' => 'Direction dure', 'icon' => 'bi-arrow-left-right'],
                ['value' => 'Freinage spongieux', 'icon' => 'bi-exclamation-triangle'],
                ['value' => 'Pédale de frein molle', 'icon' => 'bi-exclamation-triangle'],
                ['value' => 'Boîte qui accroche', 'icon' => 'bi-gear'],
                ['value' => 'Vitesse qui saute', 'icon' => 'bi-arrow-repeat'],
                ['value' => 'Clim ne refroidit pas', 'icon' => 'bi-snow'],
                ['value' => 'Consommation élevée', 'icon' => 'bi-bar-chart-line'],
                ['value' => 'Batterie faible', 'icon' => 'bi-battery'],
                ['value' => 'Lampes qui clignotent', 'icon' => 'bi-lightning'],
                ['value' => 'Véhicule immobilisé', 'icon' => 'bi-cone-striped'],
            ];
            ?>

            <div class="panne-form-grid" style="margin-bottom:1rem;">
                <div class="panne-main-panel">
                    <div class="panne-section-head">
                        <span class="panne-section-icon" aria-hidden="true"><i class="bi bi-car-front"></i></span>
                        <div>
                            <h4>Vehicule concerne</h4>
                            <p>Le rendez-vous sera rattache a ce vehicule et a votre compte client.</p>
                        </div>
                    </div>
                    <div class="sg-form-group">
                        <label for="id_vehicule">Votre vehicule *</label>
                        <select name="id_vehicule" id="id_vehicule" required>
                            <?php foreach (($clientVehicles ?? []) as $vehicle): ?>
                                <?php
                                $vehicleId = (int) ($vehicle['id_vehicule'] ?? $vehicle['id'] ?? 0);
                                $isSelected = (int) ($old['id_vehicule'] ?? ($selectedVehicleId ?? 0)) === $vehicleId;
                                $label = trim(($vehicle['marque'] ?? '') . ' ' . ($vehicle['modele'] ?? '') . ' - ' . ($vehicle['immatriculation'] ?? ''));
                                ?>
                                <option value="<?php echo $vehicleId; ?>" <?php echo $isSelected ? 'selected' : ''; ?>>
                    ...
                    <a href="index.php?action=addVehicle" class="btn-sg btn-sg-outline btn-sg-sm">
                        <i class="bi bi-plus-lg"></i> Ajouter un autre vehicule
                    </a>
                </div>
            </div>
